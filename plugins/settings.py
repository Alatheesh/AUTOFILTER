...
async def admin_input_catcher(client: Client, message: Message):
    ...
    async def finish_input(success_text, back_callback="set_home"):
        ...
        markup = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Settings", callback_data=back_callback, style=ButtonStyle.DANGER)]])
        if prompt_msg_id:
    ...
    # ⚙️ CORE ADMIN CONFIGURATIONS
        ...
        markup = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_shortener", style=ButtonStyle.DANGER)]])
        if prompt_msg_id:
...
async def menus_callback_handler(client: Client, query: CallbackQuery):
    ...
    if data == "tier_user_home":
        ...
        buttons = [
            [InlineKeyboardButton(text=f"{'✅' if m=='default' else '❌'} Default Mode", callback_data="uset_mode_default", style=ButtonStyle.PRIMARY), 
             InlineKeyboardButton(text=f"{'✅' if m=='interactive' else '❌'} Interactive Mode", callback_data="uset_mode_interactive", style=ButtonStyle.PRIMARY)],
...
async def settings_callbacks(client: Client, callback: CallbackQuery):
    ...
    # --- THE ADVANCED MODERATION HUB ---
        ...
        await callback.message.edit_text(msg, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data=back_btn, style=ButtonStyle.DANGER)]]))
    ...
    # --- CORE SETTINGS ---
        ...
        await callback.message.edit_text("✏️ **Send the trigger words in the chat.**\nSeparate them with spaces (e.g., `#example1 #sponsor2`).", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_inside", style=ButtonStyle.DANGER)]]))

    elif action == "inside_times":
        ADMIN_STATE[user_id] = {"state": "setup_inside_times", "msg_id": callback.message.id, "timestamp": time.time()}
        await callback.message.edit_text("✏️ **Send the number of verifications per day.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_inside", style=ButtonStyle.DANGER)]]))
        ...
        await callback.message.edit_text("✏️ **Send the Target Channel Usernames or IDs.**\nSeparate multiple with spaces.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_inside", style=ButtonStyle.DANGER)]]))
        ...
        if settings.get("shortener_enabled", False): await db.update_settings({"shortener_enabled": False})
            ...
            return await callback.message.edit_text("🛠 **Shortener Setup**\nSend me the **Shortener URL** in the chat.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_shortener", style=ButtonStyle.DANGER)]]))
        ...
        await callback.message.edit_text("✏️ **Send the new API Key.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_shortener", style=ButtonStyle.DANGER)]]))

    elif action == "set_url":
        ADMIN_STATE[user_id] = {"state": "waiting_for_url", "msg_id": callback.message.id, "timestamp": time.time()}
        await callback.message.edit_text("✏️ **Send the new URL Link template.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_shortener", style=ButtonStyle.DANGER)]]))
        ...
        buttons = [[InlineKeyboardButton(f"Toggle Requests {'OFF' if 'ON' in status else 'ON'}", callback_data="toggle_requests", style=ButtonStyle.PRIMARY)], [InlineKeyboardButton("🔙 Back", callback_data="set_home", style=ButtonStyle.DANGER)]]
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(buttons))
    ...
    # 🚀 NEW: MULTI-SEARCH TOGGLE HANDLER
        ...
        await callback.message.edit_text("✏️ **Send the File Auto-Delete time in MINUTES.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_autodelete", style=ButtonStyle.DANGER)]]))

    elif action == "time_filter_del":
        ADMIN_STATE[user_id] = {"state": "setup_filter_time", "msg_id": callback.message.id, "timestamp": time.time()}
        await callback.message.edit_text("✏️ **Send the Search Result Auto-Delete time in MINUTES.**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="set_autodelete", style=ButtonStyle.DANGER)]]))
    ...
    # 📝 FILE CAPTION MENUS & LOGIC
        ...
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data=f"set_caption_{scope}", style=ButtonStyle.DANGER)]]))
        ...
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data=f"set_caption_{scope}", style=ButtonStyle.DANGER)]]))
