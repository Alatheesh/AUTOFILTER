...
# 🚀 IMPORT THE KILL-SWITCH TOKEN
from plugins.search import BOT_SESSION_TOKEN
...
# 🚀 UPGRADED REGEX: Now safely unpacks Token, User ID, and DB ID!
@Client.on_callback_query(filters.regex(r"^sendfile_([^_]+)_(\d+)_(.+)$"))
async def direct_send_callback(client: Client, callback: CallbackQuery):
    ...
    db_id = callback.matches[0].group(3)
    ...
    if callback.from_user.id != searcher_id:
        return await callback.answer("⚠️ You didn't search for this movie! Please type the name to get your own file.", show_alert=True)
...
async def retry_getfile_callback(client: Client, callback: CallbackQuery):
    ...
    await execute_file_delivery(client, user_id, file_data, callback.fromuser.first_name)
